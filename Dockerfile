...
WORKDIR /ar4kAgent
RUN chmod +x gradlew
RUN ./gradlew generateBeaconctlShadowJar
RUN ./gradlew generateTemplateNifiPlugin
...
COPY --from=ar4k-builder /ar4kAgent/beacon-template-nifi-processor/build/libs/beacon-template-nifi-processor-*.nar /opt/nifi/nifi-current/lib/beacon-template-nifi-processor.nar
