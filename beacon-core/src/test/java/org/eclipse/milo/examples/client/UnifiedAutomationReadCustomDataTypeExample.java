...
/**
 * An example that shows reading the value of a node whose DataType is a custom
 * structure type.
 * <p>
 * Requires the Unified Automation CPP Demo server be running and the endpoint
 * URL be pointing to it.
 */
public class UnifiedAutomationReadCustomDataTypeExample implements ClientExample {
	...
	public void run(OpcUaClient client, CompletableFuture<OpcUaClient> future) throws Exception {
		...
		final DataValue dataValue = client
				.readValue(0.0, TimestampsToReturn.Neither, NodeId.parse("ns=2;s=Demo.Static.Scalar.WorkOrder")).get();

		final ExtensionObject xo = (ExtensionObject) dataValue.getValue().getValue();

		final Object value = xo.decode(client.getDynamicSerializationContext());
